...
# extracts the bytes from baserom.z64 (hash-checked) and writes
...
if [ -f "baserom.z64" ] && [ ! -f "aspmain_combined.bin" ]; then
    if command -v python3 >/dev/null 2>&1; then
        python3 tools/build_aspmain_combined.py || \
            echo "WARNING: build_aspmain_combined.py failed; rebuild manually before configuring CMake"
    else
        echo "WARNING: python3 not found; run tools/build_aspmain_combined.py manually before configuring CMake"
    fi
fi
